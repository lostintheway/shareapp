...
func (uq *UserQuery) QueryPortfolios() *PortfolioQuery {
	query := &PortfolioQuery{config: uq.config}
...
func (uq *UserQuery) WithPortfolios(opts ...func(*PortfolioQuery)) *UserQuery {
	query := &PortfolioQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withPortfolios = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withPortfolios; query != nil {
...
func (uq *UserQuery) loadPortfolios(ctx context.Context, query *PortfolioQuery, nodes []*User, init func(*User), assign func(*User, *Portfolio)) error {
	...
	query.Where(predicate.Portfolio(func(s *sql.Selector) {
		s.Where(sql.InValues(user.PortfoliosColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_portfolios" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
