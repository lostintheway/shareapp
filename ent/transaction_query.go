...
// QueryPfID chains the current query on the "pf_id" edge.
func (tq *TransactionQuery) QueryPfID() *PortfolioQuery {
	query := &PortfolioQuery{config: tq.config}
...
// WithPfID tells the query-builder to eager-load the nodes that are connected to
// the "pf_id" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TransactionQuery) WithPfID(opts ...func(*PortfolioQuery)) *TransactionQuery {
	query := &PortfolioQuery{config: tq.config}
...
//		GroupBy(transaction.FieldShareName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Transaction.Query().
//		Select(transaction.FieldShareName).
//		Scan(ctx, &v)
...
func (tq *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if query := tq.withPfID; query != nil {
		if err := tq.loadPfID(ctx, query, nodes, nil,
			func(n *Transaction, e *Portfolio) { n.Edges.PfID = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TransactionQuery) loadPfID(ctx context.Context, query *PortfolioQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Portfolio)) error {
